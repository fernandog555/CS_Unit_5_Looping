...
// If the stock number is not 209, 312, or 414 the user must reenter the
// number The program displays the correct price
using System.IO;
using System;
...
class DebugFive2
{
  ...
  Main ()
  {
    ...
    stockNum = ReadLine ();
    while (!(stockNum.Equals (ITEM209) || stockNum.Equals (ITEM312)
             || stockNum.Equals (ITEM414)))
      {
    ...
    if (stockNum == ITEM209)
      ...
      price = PRICE414;
    WriteLine ("The price for item # {0} is {1}", stockNum,
               price.ToString ("C"));
  }
}
